...
namespace dlibx {
    ...
    /** \brief Alternate version of dlib::loss_metric_
     * with dynamic threshold selection.
     */
    class loss_metric_dynamic_ {
    public:
        using training_label_type = unsigned long;
        using output_label_type = dlib::matrix<float,0,1>;

        loss_metric_dynamic_() = default;

        loss_metric_dynamic_(float margin) : margin(margin) {
        ...
        friend void serialize(
            const loss_metric_dynamic_& item, std::ostream& out) {
            dlib::serialize("loss_metric_dynamic_1", out);
            dlib::serialize(item.margin, out);
        }

        friend void deserialize(loss_metric_dynamic_& item, std::istream& in) {
            std::string version;
            dlib::deserialize(version, in);
            if (version == "loss_metric_dynamic_1") {
                dlib::deserialize(item.margin, in);
            }
            else if (version == "loss_metric_") {
                // These values used to be hard coded, so for this version
                // of the metric learning loss we just use these values.
                item.margin = 0.8f;
            }
            else if (version == "loss_metric_2") {
                dlib::deserialize(item.margin, in);
                double dist_thresh;
                dlib::deserialize(dist_thresh, in); // ignored
            }
            else
                throw dlib::serialization_error("Unexpected version found while deserializing dlib::loss_metric_dynamic_.  Instead found " + version);
        }

        friend std::ostream& operator<<(std::ostream& out, const loss_metric_dynamic_& item ) {
            out << "loss_metric (margin="<<item.margin<<")";
            return out;
        }

        friend void to_xml(const loss_metric_dynamic_& item, std::ostream& out) {
            out << "<loss_metric margin='"<<item.margin<<"'/>";
        }
    ...
    private:
        ...
        // These variables are only here to avoid being reallocated over
        // and over in compute_loss_value_and_gradient()
        mutable dlib::resizable_tensor temp, grad_mul;
    ...
    using loss_metric = dlib::add_loss_layer<loss_metric_dynamic_, SUBNET>;
