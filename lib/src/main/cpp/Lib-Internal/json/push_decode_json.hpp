...
namespace json {
    namespace detail {
        decoder_input_fn push_decode_json(
            decoder_output_fn fn, std::shared_ptr<exception_handler_base> eh);
    }

    /** \brief Decode character stream to json value stream.
     *
     ...
     */
    inline decoder_input_fn push_decode_json(decoder_output_fn fn) {
        return detail::push_decode_json(fn,nullptr);
    }
    ...
    inline decoder_input_fn push_decode_json(decoder_output_fn fn, HANDLER h) {
        return detail::push_decode_json(
